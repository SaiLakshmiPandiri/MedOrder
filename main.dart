...
// ---------- HOME PAGE ----------
enum SortBy { none, priceLowHigh, priceHighLow, nameAZ, 
nameZA } class HomePage extends StatefulWidget {
...
description: 'Effective for fever and mild 
pain.', price: 39,
category: 'Painkiller',
...
description: 'Boosts immunity and reduces cold 
duration.', price: 129,
...
description: 'Fast relief for acidity and 
indigestion.', price: 59,
category: 'Digestive',
...
description: 'Broad-spectrum antibiotic for bacterial
infections.', price: 149,
category: 'Antibiotic',
...
description: 'Allergy relief for sneezing and runny 
nose.', price: 25,
category: 'Cough & Cold',
...
description: 'Soothes sore throat and 
cough.', price: 85,
category: 'Cough & Cold',
...
'm12',
name: 'Aloe Vera Gel',
description: 'Hydrating gel for skin healing and 
soothing.', price: 145,
category: 'Skincare',
...
['All', ..._allMeds.map((e) => e.category).toSet()];
List<Medicine> get displayedMeds { 
List<Medicine> list = _allMeds.where((m) 
{ final matchesCat =
_selectedCategory == 'All' || m.category ==
_selectedCategory; final matchesSearch =
...
case SortBy.nameAZ:
list.sort((a, b) => 
a.name.compareTo(b.name)); break;
case SortBy.nameZA:
list.sort((a, b) => 
b.name.compareTo(a.name)); break;
default: 
break;
}
return list;
...
setState(() {
final index = cart.indexWhere((c) => c.medicine.id == 
med.id); if (index >= 0) {
cart[index].qty++;
} else {
...
Bar( SnackBar(
content: Text('${med.name} added to 
cart'), duration: const
...
{ setState(() {
final i = cart.indexWhere((c) => c.medicine.id == 
id); if (i >= 0) {
cart[i].qty += delta;
...
.showSnackBar(const SnackBar(content: Text('Cart is 
empty!'))); return;
}
showDialog( 
context: context,
...
content: Text('Your total is 
₹${total.toStringAsFixed(0)}'), actions: [
TextButton(
onPressed: () => 
Navigator.pop(context), child: const 
Text('Cancel')),
ElevatedButton(
onPressed: () { 
...
context) { final meds = 
displayedMeds;
final cartCount = cart.fold<int>(0, (p, c) => p + c.qty); return Scaffold(
appBar: AppBar(
title: const Text('MedOrder'), 
actions: [ 
PopupMenuButton<SortBy>(
icon: const Icon(Icons.sort),
onSelected: (v) => setState(() => _sortBy = 
v), itemBuilder: (_) => const [
...
prefixIcon: const
Icon(Icons.search), hintText: 
'Search medicines...', filled: true,
...
vertical: 6), itemCount: meds.length,
itemBuilder: (_, i)
{ final m =
meds[i]; return 
Card(
...
backgroundColor: 
Colors.teal.shade400, foregroundColor: 
Colors.white,
...
final void Function(String, int) 
onQtyChange; final void Function(String) 
onRemove;
final double total;
...
onPressed: () 
=>
onQtyChange(item.medicine.id, -1),
...
padding:
const EdgeInsets.symmetric(horizontal: 16, vertical: 
8), child: ElevatedButton.icon(
onPressed: onCheckout,
icon: const 
Icon(Icons.payments_outlined), label: 
const Text('Checkout'),
style: ElevatedButton.styleFrom( 
minimumSize: const 
Size.fromHeight(50), 
